...
def todos_alunos():
    with engine.connect() as con:  # conectar ao banco de dados
        ...
        while True:
            ...
            if result == None:
                break
            # d_result = {"id": result[0], "nome": result[1], "email": result[2]}
...
def criar_livros(id_livro,  descricao):
    with engine.connect() as con:
        sql_create = text(
...
# INSERT INTO Livro (id_livro, descricao) VALUES (4, "As seis lições,  por Ludwig von Mises")
# print('cria livros:', criar_livros(5, "Star Wars"))
...
"SELECT id_livro, id_aluno, descricao FROM Livro JOIN Aluno ON livro.id_aluno = aluno.id WHERE nome = 'Mirtes'"


def livros_do_aluno(nome):
    with engine.connect() as con:
        sql_consulta = text('''SELECT id_livro, id_aluno, descricao
                            FROM Livro JOIN Aluno ON
...
print('********************************************')
print('todos os livros parados:', livros_parados())
